...
def run_tests():
    ...
    print(f"[OK] Timeline metadata verified: {timeline_meta['start_date']} to {timeline_meta['end_date']} with {len(timeline_meta['milestones'])} milestones", flush=True)
    ...
    print(f"[OK] 3D Depth layers verified: {len(depth_layers)} depth zones from 0m to 2000m", flush=True)
    ...
    print(f"[OK] Snapshot at Start (t=0%): {snapshot_start['active_platforms_count']} active platforms", flush=True)
    ...
    print(f"[OK] Snapshot at Midpoint: time_progress={snapshot_mid['time_progress_pct']}%, Mean basin temp={snapshot_mid['mean_basin_temp']} C", flush=True)

    snapshot_end = SpatiotemporalEngine.get_state_at_time("2026-09-04T12:00:00Z")
    assert snapshot_end["time_progress_pct"] == 100.0
    print(f"[OK] Snapshot at Real-Time End: {snapshot_end['argo_floats'][0]['name']} at lat={snapshot_end['argo_floats'][0]['lat']}, lon={snapshot_end['argo_floats'][0]['lon']}", flush=True)
    ...
    print("[OK] GET /api/v1/visualization/timeline returned 200 OK", flush=True)

    res_dl = client.get("/api/v1/visualization/depth-layers")
    assert res_dl.status_code == 200
    assert len(res_dl.json()) >= 7
    print("[OK] GET /api/v1/visualization/depth-layers returned 200 OK", flush=True)
    ...
    print("[OK] GET /api/v1/visualization/state-at-time returned 200 OK", flush=True)
